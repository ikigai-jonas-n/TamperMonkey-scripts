// ==UserScript==
// @name         [7.85] IKG Attendance Pro (Autopilot & Alarms)
...
(function () {
    ...
    const cleanUpAndMigrateStorage = () => {
        ...
        // 1. MIGRATION: Auto-migrate highest old versions to V9 so you don't lose downloaded data
        const baseKeys = [
        ...
        baseKeys.forEach(base => {
            ...
            if (!localStorage.getItem(newKey)) {
                ...
                let highestOldVer = -1;
                // Find the highest existing version for this base key
                ...
                // Copy to new V9 key if found
                if (highestOldVal !== null) {
                    localStorage.setItem(newKey, highestOldVal);
                    IkgLog.info(`Migrated ${base}_V${highestOldVer} -> ${newKey}`);
                }
            }
        });

        // 2. GARBAGE COLLECTION: Nuke expired daily keys & ALL outdated V-keys
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            // Clean up old daily UI flags (from yesterday or older)
            const dailyMatch = key.match(dailyPattern);
            if (dailyMatch) {
                const datePart = dailyMatch[2];
                if (datePart < todayStr) keysToRemove.push(key);
                continue;
            }

            // Clean up outdated version keys (e.g. V1, V6, V8)
            const verMatch = key.match(versionPattern);
            if (verMatch) {
                // If it ends with a version number but it's NOT our current APP_VER, NUKE IT.
                if (!key.endsWith(`_${APP_VER}`)) {
        ...
        if (keysToRemove.length > 0) IkgLog.info(`Garbage Collector: NUKED ${keysToRemove.length} expired or deprecated keys.`);
    };
    ...
    const safeFloat = (num) => Math.round((num + Number.EPSILON) * 100) / 100;
    ...
    const evaluateDay = (record, note) => {
        ...
        // 🎯 CRITICAL FIX: Override Deel's 8h rule. Pad Full PTO to 9.0h 
        // so it perfectly hits the dotted target line on your Analytics chart.
        if (isFullPTO) ptoHrs = 9.0;

        const actualHrs = (record && record.workHours) ? parseFloat(record.workHours) : 0;
        const targetHrs = 9.0;

        // Now effectiveHrs natively equals 9.0 for full PTO days
        ...
        if (isFullPTO) {
        ...
        } else if (actualHrs === 0 && record && record.startTime) {
            ...
            if (isPartialPTO) {
                if (effectiveHrs >= targetHrs) {
                    ...
                    reason = `Goal passed with +${ptoHrs}h PTO`;
                } else {
                    status = 'partial-pto-fail';
                    color = 'var(--danger)';
                    chartColor = '#EF4444';
                    heatmapBg = 'linear-gradient(135deg, #EF4444 50%, #8B5CF6 50%)';
                    reason = `Short by ${formatDurFromDec(Math.abs(flexHrs), false)}`;
                }
            } else {
                if (actualHrs >= targetHrs) {
                    status = 'pass'; color = 'var(--success)'; chartColor = '#10B981'; heatmapBg = '#10B981'; reason = 'Goal met';
                } else {
                    status = 'fail'; color = 'var(--danger)'; chartColor = '#EF4444'; heatmapBg = '#EF4444'; reason = `Short by ${formatDurFromDec(Math.abs(flexHrs), false)}`;
                }
            }
        }
        ...
        if ((status === 'pass' || status === 'partial-pto-pass') && actualHrs >= 10.0) {
            chartColor = '#059669';
            heatmapBg = status === 'pass' ? '#059669' : 'linear-gradient(135deg, #059669 50%, #8B5CF6 50%)';
        }

        return { isFullPTO, isPartialPTO, ptoHrs, ptoType, actualHrs, effectiveHrs, targetHrs, flexHrs, status, color, chartColor, heatmapBg, reason };
    };
    ...
    const drawNativeChart = (canvasId, labels, dataItems) => {
        ...
        const targetY = padTop + chartH - (9.0 / maxVal) * chartH;
        ctx.beginPath(); ctx.setLineDash([5, 5]); ctx.moveTo(padLeft, targetY); ctx.lineTo(padLeft + chartW, targetY);
        ...
        dataItems.forEach((item, i) => {
            ...
            // 1. Draw Actual Worked Hours (Bottom)
            if (item.val > 0) {
            ...
            // 3. Draw Combined Total Text
            const totalVal = item.val + item.pto;
            if (dataItems.length <= 40 && totalVal > 0) {
    ...
    GM_addStyle(`
        ...
        #ikg-chart-pane { flex: 2.5; border-right: 1px solid var(--border); background: var(--bg-base); padding: 32px; display: flex; flex-direction: column; overflow: hidden; }
    ...
    const updateActiveShiftUI = () => {
        ...
        for (let i = 1; i <= 31; i++) {
            ...
            const evalDay = evaluateDay(record, dayNotes[dStr]);
        ...
        // 🎯 CRITICAL FIX: Safe float math based on the helper
        const todaysEval = evaluateDay(localCache[todayStr], dayNotes[todayStr]);
    ...
    function renderCalendar() {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            const evalDay = evaluateDay(record, dayNotes[dateStr]);
            ...
            if (isFetchingData && dateStr <= todayStr && record === undefined && !evalDay.isFullPTO) {
                ...
                let pendingIcon = '';
                let timesClass = '';
                let outTimeDisplay = formatTime(record.endTime);
                ...
                // 🎯 UI FIX: Move pill to the date header. `margin-right: auto` pushes it left, date stays right.
                ...
                cellContent = `
                    <div class="ikg-cell-data">
                        ...
                        <div class="ikg-times ${timesClass}" style="margin-top:auto; ${activeShiftStyles}">
                            <div>IN <span>${formatTime(record.startTime)}</span></div>
    ...
    function renderAudit(localCache) {
        ...
        filteredDates.forEach(dateStr => {
            ...
            if (record && record.startTime && record.endTime) {
                ...
                const exactSecs = Math.round((record.endTime - record.startTime) / 1000);
                const exactHrs = exactSecs / 3600;

                const exactFlexHrs = (exactHrs + evalDay.ptoHrs) - 9.0;
                const flexStr = exactFlexHrs === 0 ? '-' : formatDurFromDec(exactFlexHrs, true);

                // 🎯 MINIMALIST AUDIT UI (Injects full PTO type)
    ...
    function renderAnalytics(localCache) {
        ...
        filteredDates.forEach(dateStr => {
            ...
            const evalDay = evaluateDay(record, note);
            const hrs = record ? parseFloat(record.workHours) : 0;
        ...
        if (currentStatsRange === 'ALL') {
            ...
            if (globalFirstDate !== "--") {
                let start = new Date(globalFirstDate !== "--" ? globalFirstDate : todayStr);
                start.setHours(0, 0, 0, 0); start.setDate(start.getDate() - start.getDay());
                let end = new Date(todayStr); end.setHours(0, 0, 0, 0); end.setDate(end.getDate() + (6 - end.getDay()));
                ...
                for (let c = 0; c < totalCols; c++) {
                    ...
                    for (let r = 0; r < 7; r++) {
                        ...
                        const evalDay = evaluateDay(rec, note);
                        ...
                        else if (evalDay.status === 'full-pto') { color = evalDay.heatmapBg; title += ` | ${evalDay.ptoType}`; innerTxt = 'PTO'; }
                        else if (evalDay.actualHrs > 0) {
                            color = evalDay.heatmapBg;
                            if (evalDay.isPartialPTO) title += ` | ${evalDay.effectiveHrs.toFixed(2)}h (${evalDay.actualHrs.toFixed(1)}h worked + ${evalDay.ptoHrs}h ${evalDay.ptoType})`;
                            else title += ` | ${evalDay.actualHrs.toFixed(2)}h`;
                            innerTxt = evalDay.effectiveHrs.toFixed(1);
                        } else if (evalDay.status === 'pending') { color = evalDay.heatmapBg; title += ' | Pending'; innerTxt = '⌛'; }

                        heatmapGrid.innerHTML += `
                            <div class="ikg-heat-sq ikg-fast-tt" style="background:${color}; ${pointerEvent}" data-title="${title}" data-jump="${dStr}">
                    ...
                    if (isNewMonth) {
                        ...
                        monthsContainer.innerHTML += `<div style="position:absolute; left:${leftPercent}%; font-size:11px; color:var(--text-muted); font-weight:500;">${monthName}</div>`;
                    }
                }
            }
        } else {
            canvasContainer.style.display = 'block'; heatmapWrapper.style.display = 'none';
            ...
            filteredDates.forEach(dStr => {
                ...
                const evalDay = evaluateDay(localCache[dStr], dayNotes[dStr]);
                dataItems.push({ val: evalDay.actualHrs, pto: evalDay.ptoHrs, color: evalDay.chartColor });
            });
    ...
    function buildUI() {
                ...
                <div id="ikg-modal-body">
                    <div id="view-cal" class="ikg-view active">
                        <div id="ikg-calendar-pane">
                            ...
                            <div id="ikg-day-modal" class="ikg-popup-modal">
                                <div class="dm-title" id="dm-title-date">Date</div>
                                <div class="dm-toggle" id="dm-pto-toggle">
                                    <span style="font-weight:600; color:var(--text-main);">🏝️ Mark as PTO / Leave</span>
                                    <span style="font-size:18px;">✓</span>
                                </div>
                                <button class="dm-btn" id="dm-save-btn">Save Changes</button>
                            </div>
                        </div>
                    ...
                    <div id="view-stats" class="ikg-view">
                        <div id="ikg-chart-pane">
                            ...
                            <div class="ikg-canvas-container"><canvas id="ikg-main-chart"></canvas></div>
        ...
        document.getElementById('ikg-heatmap-grid').addEventListener('click', (e) => {
            if (e.target.classList.contains('ikg-heat-sq')) {
                const jumpStr = e.target.getAttribute('data-jump');
                if (jumpStr) {
                    currentViewYear = new Date(jumpStr).getFullYear(); currentViewMonth = new Date(jumpStr).getMonth() + 1;
                    document.getElementById('tab-cal').click(); renderCalendar();
                }
            }
        });
        ...
        document.getElementById('dm-pto-toggle').addEventListener('click', (e) => { e.currentTarget.classList.toggle('active'); });
        document.getElementById('dm-save-btn').addEventListener('click', () => {
            const isPTO = document.getElementById('dm-pto-toggle').classList.contains('active');
            let notes = JSON.parse(localStorage.getItem(DAY_NOTES_KEY) || '{}');
            notes[activeModalDate] = { isPTO: isPTO };
            localStorage.setItem(DAY_NOTES_KEY, JSON.stringify(notes));
            IkgLog.info(`Saved PTO status for ${activeModalDate}`, { isPTO });
            dmOverlay.classList.remove('open'); dmModal.classList.remove('open');
            renderCalendar();
            if (activeTab === 'stats') renderAnalytics(JSON.parse(localStorage.getItem(CACHE_KEY) || '{}'));
            if (activeTab === 'audit') renderAudit(JSON.parse(localStorage.getItem(CACHE_KEY) || '{}'));
        });
        ...
        ['set-sound', 'set-img-type', 'set-pulse-speed', 'set-snooze', 'set-snooze-unit', 'set-trigger-unit', 'set-use-flex', 'set-show-secs', 'set-auto-alarm', 'set-manual-shift', 'set-volume'].forEach(id => { document.getElementById(id).addEventListener('change', autoSaveSettings); });
        document.getElementById('set-triggers').addEventListener('change', autoSaveSettings);
        ...
        document.getElementById('ikg-btn-fetch').addEventListener('click', async () => {
            ...
            if (deelPto && Object.keys(deelPto.ptoCalendar).length > 0) {
                Object.keys(deelPto.ptoCalendar).forEach(dateStr => {
                    ...
                    dayNotes[dateStr] = {
                        isPTO: ptoInfo.isFullDay, // Only triggers the massive pill if it's a full day
                        isPartialPTO: !ptoInfo.isFullDay, // Flags partial hours
                        source: 'Deel',
                        type: ptoInfo.type,
                ...
                if (deelPto.syncLog.length > 0) IkgLog.info("Deel Manual Adjustments Found:", deelPto.syncLog);
            ...
            globalTotalDays = 0; globalTotalHours = 0;
